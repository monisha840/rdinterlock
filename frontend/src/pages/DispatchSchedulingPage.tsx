...
const STATUS_OPTIONS = ["SCHEDULED", "READY", "DISPATCHED"];
...
const DispatchSchedulingPage = () => {
    ...
    const [form, setForm] = useState({ clientId: "", brickTypeId: "", quantity: "", location: "", dispatchDate: new Date().toISOString().split("T")[0], driverId: "", status: "SCHEDULED", notes: "" });
    ...
    const createMut = useMutation({
        ...
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["dispatch-schedules"] }); setShowModal(false); resetForm(); toast.success("✅ Schedule created"); },
        onError: (e: any) => toast.error("❌ Failed", { description: e.message }),
    });

    const updateMut = useMutation({
        mutationFn: ({ id, data }: any) => clientsApi.updateSchedule(id, data),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["dispatch-schedules"] }); setShowModal(false); setEditing(null); resetForm(); toast.success("✅ Schedule updated"); },
        onError: (e: any) => toast.error("❌ Failed", { description: e.message }),
    });

    const deleteMut = useMutation({
        mutationFn: (id: string) => clientsApi.deleteSchedule(id),
        onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["dispatch-schedules"] }); toast.success("✅ Schedule deleted"); },
    });

    const markDispatched = (id: string) => updateMut.mutate({ id, data: { status: "DISPATCHED" } });

    const resetForm = () => setForm({ clientId: "", brickTypeId: "", quantity: "", location: "", dispatchDate: new Date().toISOString().split("T")[0], driverId: "", status: "SCHEDULED", notes: "" });

    const openEdit = (s: any) => {
        setEditing(s);
        setForm({
            clientId: s.clientId, brickTypeId: s.brickTypeId, quantity: String(s.quantity), location: s.location || "",
            dispatchDate: new Date(s.dispatchDate).toISOString().split("T")[0], driverId: s.driverId || "", status: s.status, notes: s.notes || "",
        });
        setShowModal(true);
    };
    ...
    const handleSubmit = () => {
        ...
        const payload = {
            clientId: form.clientId, brickTypeId: form.brickTypeId, quantity: parseInt(form.quantity),
            location: form.location || undefined, dispatchDate: form.dispatchDate,
            driverId: form.driverId || undefined, status: form.status, notes: form.notes || undefined,
        };
        if (editing) updateMut.mutate({ id: editing.id, data: payload });
        else createMut.mutate(payload);
    };

    // Upcoming reminders (within 3 days and not dispatched)
    const now = new Date();
    const threeDays = new Date(now.getTime() + 3 * 86400000);
    const upcoming = schedules.filter((s: any) => new Date(s.dispatchDate) <= threeDays && s.status !== "DISPATCHED");
                    ...
                    <div className="bg-card rounded-2xl p-6 w-full max-w-md border border-border shadow-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="Location" className="h-10 px-3 bg-secondary/50 border border-border rounded-xl text-sm" />
                            </div>
